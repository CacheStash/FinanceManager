...
const Reports: React.FC<ReportsProps> = ({ transactions, accounts, lang = 'en', marketData }) => {
  ...
  const CASHFLOW_COLORS = ['#10b981', '#f43f5e']; // Green for Income, Red for Expense
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
      ...
      {/* --- 2. GLOBAL MARKET STATS (POSISI DI BAWAH SUMMARY) --- */}
      {marketData && (
      ...
      <div className="bg-surface p-5 rounded-2xl border border-white/10 h-[380px] flex flex-col">
          ...
          <div className="flex-1 relative">
                  ...
                  <PieChart>
                      ...
                      >
                          {cashflowData.map((entry, index) => (
                              <Cell key={`cell-${index}`} fill={CASHFLOW_COLORS[index]} stroke="rgba(0,0,0,0.2)" strokeWidth={2} />
                          ))}
                      </Pie>
                      <Tooltip contentStyle={{ backgroundColor: '#18181b', borderColor: '#333', borderRadius: '12px', color: '#fff' }} itemStyle={{ color: '#fff' }} formatter={(val: number) => formatCurrency(val)} />
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none pb-8">
                  <span className="text-[10px] text-gray-500 uppercase font-bold">Net Flow</span>
